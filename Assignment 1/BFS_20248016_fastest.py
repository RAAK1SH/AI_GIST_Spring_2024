...
class VacuumCleaner:
    ...
    def actuator_BFS(self): 
        ...
        # Queue for BFS 
        queue = deque()
        queue.append((self.location_sensor(), []))  # Each queue item stores a position and its path
        
        while queue:
            current_position, current_path = queue.popleft()  # Dequeue a cell and its path
           ...
           # Check if the current position
            ...
            # This can be the fastest way to solve problem 
            # Redraw the grid and vacuum cleaner
            screen.fill(black)
            draw_grid_and_vacuum()  
            pygame.display.flip()
            ...
            # print(f"Current Vacuum Position: {current_pos_label} -> Target Square Reached")
            ...
            for neighbor in neighbors:
                if 0 <= neighbor[0] < grid_height and 0 <= neighbor[1] < grid_width:
                    if neighbor not in self.visited:  # Check neighbor
                        queue.append((neighbor, current_path + [current_position]))  # Enqueue with updated path
                        # self.visited[neighbor] = True  # Mark neighbor as visited
